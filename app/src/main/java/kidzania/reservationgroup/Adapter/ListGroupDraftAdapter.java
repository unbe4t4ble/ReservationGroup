...
import kidzania.reservationgroup.ViewHolder.GroupViewDraftHolder;
...
public class ListGroupDraftAdapter extends ListGroupAdapter{

    ListGroupDraftAdapter(List<GroupModel> data) {
        super(data);
    }

    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_group_list_draft, parent, false);
        return new GroupViewDraftHolder(view);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof GroupViewDraftHolder) {
            final GroupViewDraftHolder myHolder = (GroupViewDraftHolder) holder;
