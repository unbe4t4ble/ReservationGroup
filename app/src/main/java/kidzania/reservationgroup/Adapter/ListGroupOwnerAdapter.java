...
import kidzania.reservationgroup.ViewHolder.GroupViewOwnerHolder;
...
public class ListGroupOwnerAdapter extends ListGroupAdapter{

    private ArrayList<String> idNumEscAdapter = new ArrayList<>();

    ListGroupOwnerAdapter(List<GroupModel> data) {
        super(data);
    }

    @Override
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_group_list_owner, parent, false);
        return new GroupViewOwnerHolder(view);
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof GroupViewOwnerHolder) {
            final GroupViewOwnerHolder myHolder = (GroupViewOwnerHolder) holder;
